...
const getAllPosts = async id => {
  const response = await fetch(`https://docs.google.com/spreadsheets/d/${id}/gviz/tq?tqx=out:json&sheet=Posts`);
  ...
  for (var i=0; i<rows.length; i++) {
    ...
    posts[i].prettyDate = posts[i].date.toLocaleDateString("en-GB",{weekday:"long",year:"numeric",month:"long",day:"numeric"})
    ...
    posts[i].slug = posts[i].title ? encodeURIComponent(posts[i].title) : Date.now();
    
    /* ::: Save the blog ID, so we can reference the parent blog's URL from within the post ::: */
    posts[i].parent = id;
  }
...
const getCSS = async id => {
  const response = await fetch(`https://docs.google.com/spreadsheets/d/${id}/gviz/tq?tqx=out:json&sheet=CSS`);
...
const getSEO = async id => {
  var seo = {};
  const response = await fetch(`https://docs.google.com/spreadsheets/d/${id}/gviz/tq?tqx=out:json&sheet=SEO`);
